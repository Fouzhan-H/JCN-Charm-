...
void PyramidTree::Initialize(int col, const double *bounds)
{
  this->RawData.clear();
  this->SumData.clear();
  this->Map.clear();
...
int PyramidTree::GetNumberElement()
{
  return this->RawData.size();
}

//-----------------
int PyramidTree::GetNumberDimension()
{
  return this->ColNumber;
}

//----------------
void PyramidTree::SetDimension(int dim)
{
  this->ColNumber = dim;
}

//------------------
void PyramidTree::SetMaxBounds(const double *bounds)
{
  for (int i = 0; i < this->ColNumber; i++)
     {
...
//------------------
void PyramidTree::SetMinBounds(const double *bounds)
{
  for (int i = 0; i < this->ColNumber; i++)
    {
...
//------------------
int PyramidTree::InsertUniquePoint(const double *point, IdType &pid)
{
...
void PyramidTree::InsertToStructure(std::vector<double> point, IdType duplicate)
{
...
//------------------
void PyramidTree::GetPointIndex(IdType &pid, const double *point)
{
  int finalIndex;
...
//----------------------------------------------------------------------------
void PyramidTree::GetPoint(IdType pid, double *point)
{
...
int PyramidTree::GetNumberOfPoints()
{
  return this->RawData.size();
}

//----------------------------------------------------------------------------
void PyramidTree::InsertNextPoint(const double *point)
{
...
//----------------------------------------------------------------------------
double PyramidTree::GetTime()
{
    return TTime;
}

//----------------------------------------------------------------------------
int PyramidTree::ComputePTStruture(std::vector<double> myPoint)
{
  // Transform the original data to the encoded data sets.
  double tempValue[this->ColNumber];
  for (int j = 0; j < this->ColNumber; j++)
    {
    tempValue[j] = (myPoint[j] - this->MinDim[j]) / (this->MaxDim[j] - this->MinDim[j]);
    }
    
  // Calculate the interpolted 1-D point
  int dMax = 0;
  double diff; 
  double height = fabs(0.5 -tempValue[0]);
  int index;
  for (int j = 1; j < this->ColNumber; j++){
    diff = fabs(0.5-tempValue[j]);
    if (compare(height, diff) == -1){  // (height < diff)
      dMax = j;
      height = diff;
    }
  }
...
//----------------------------------------------------------------------------
int PyramidTree::HashValue(std::vector<double> point)
{
  int searchKey = 0 ;
...
int PyramidTree::GetBucketCount()
{
  return this->Map.bucket_count();  
}

int PyramidTree::GetBucketSize()
{
  int sum = 0;
