...
#ifndef PYRAMIDTREE_H
#define PYRAMIDTREE_H
...
public:
  ...
  int InsertUniquePoint(const double *point, IdType &pid);

  // Description: 
  // Retrieve a point with a given id
  void GetPointIndex(IdType &pid, const double *point);
  ...
  void GetPoint(IdType pid, double *point);
...
private:  
  ...
  void InsertToStructure(std::vector<double> myPoint, IdType dupliate );
  ...
  std::unordered_map<int, std::vector<int>,ihash> Map;
